...
class HashVerifierApp:
    def __init__(self, page: ft.Page):
        ...
        self.hashes: dict[str, str] = {}  # {"MD5": "...", "SHA-1": "...", "SHA-256": "..."}
        ...
        # FilePicker" (uma barra vermelha de erro) — não é um elemento visual,
        # é mesmo um bug de temporização. Por isso criamos o controlo aqui,
        # mas só o registamos no overlay no final do __init__.
       # ---------------- Serviços ----------------
        self.file_picker = ft.FilePicker()
        self.page.overlay.append(self.file_picker) # Registamos logo aqui, como na versão antiga
        self.page.update() # Forçamos a atualização para o overlay aceitar
        # ---------------- Controlos globais ----------------
        ...
        self.file_size_text = ft.Text("", italic=True, color=ft.Colors.GREY_700, size=12)
        ...
        self.page.add(
            ft.Column(
                expand=True,
                spacing=16,
                controls=[self.header, self.tabs],
            )
        )
    ...
    async def calculate_hashes(self, data: bytes, t: dict):
        ...
        total = len(data) or 1
        chunk_size = max(65536, total // 100)  # ~100 atualizações de progresso
        processed = 0
        ...
        for start in range(0, len(data), chunk_size):
            ...
            self.page.update()
            # Cede o controlo ao loop de eventos para a UI não bloquear
    ...
    async def _export(self, filename: str, content: bytes):
        t = TEXT[self.lang]
        try:
            path = await self.file_picker.save_file(file_name=filename, src_bytes=content)
    ...
    def apply_translations(self):
        ...
        if self.selected_file_size is not None:
            self.file_size_text.value = f"{t['size_prefix']}{human_size(self.selected_file_size)}"
...
def main(page: ft.Page):
    # 1. Configurações iniciais de página
    page.title = "Verificador"
    
    # 2. Criamos a aplicação (mas NÃO a adicionamos à página ainda)
    app = HashVerifierApp(page)
    
    # 3. Adicionamos a app à página
    page.add(ft.Column(controls=[app.header, app.tabs]))
    
    # 4. SÓ AGORA adicionamos o FilePicker ao overlay
    # Isto garante que o handshake da página já aconteceu e a barra não aparece

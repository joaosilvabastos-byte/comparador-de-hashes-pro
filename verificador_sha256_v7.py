...
class HashVerifierApp:
    def __init__(self, page: ft.Page):
        ...
        self.hash_copy_buttons = {
            algo: ft.IconButton(
                icon=ft.Icons.COPY,
                on_click=lambda e, a=algo: self._copy_hash(a),
            )
       ...
       # VIEW DE HISTÓRICO
        ...
        self.header = ft.Container(
            padding=ft.padding.only(16, 14, 16, 14),
            border_radius=12,
                ...
                controls=[
                    ft.Row(
                        controls=[
                            ...
                            ft.Text(TEXT[self.lang]["title"], color=ft.Colors.WHITE, size=20, weight="bold"),
                        ]
                    ),
                    self.lang_dropdown,
                ],
    ...
    async def _on_file_picked(self, e: ft.FilePickerResultEvent):
        ...
        # Lê o ficheiro diretamente pelo caminho (path)
        with open(file.path, "rb") as f:
            file_bytes = f.read()
        
        # Agora sim, chama a função de cálculo passando os bytes que leste
        t = TEXT[self.lang]
        self.page.run_task(self._calculate_hashes, file_bytes, t)
    ...
    async def _calculate_hashes(self, data, t):
        ...
        # Correção: total = len(data) or 1 evita divisão por zero
        total = len(data) or 1
        ...
        # Atualização robusta para garantir que os números aparecem
        for a, f in self.hash_fields.items():
            f.value = self.hashes[a]
            f.update() # Força a atualização do campo de texto

        self.progress_bar.visible = False
        self.progress_text.value = "" # Ou t["done"]
        self.page.update()
    ...
    def _compare_hashes(self, e):
        ...
        self.status_text.color = ft.Colors.GREEN if match else ft.Colors.RED
    ...
    def _refresh_history(self):
        ...
        for h in self.history:
            sym = "✅" if h["match"] else "❌"
            col = ft.Colors.GREEN if h["match"] else ft.Colors.RED
   ...
   # LIMPAR HISTÓRICO
    def _clear_history(self, e):
        ...
        def do_clear(ev):
            ...
            # --- CORREÇÃO AQUI ---
            # Limpa o texto de status e reseta a cor
            self.status_text.value = ""
